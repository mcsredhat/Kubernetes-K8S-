...
check_prereqs() {
  ...
  # Check cluster connectivity
    ...
    # Load variables
    load_env
    # Check cloud provider is set (optional for local storage)
    if [[ -z "${CLOUD_PROVIDER:-}" ]]; then
        log_warning "CLOUD_PROVIDER not set - volume snapshots will be disabled"
        log_info "This is OK for local storage. File-based backups will still work."
    elif [[ "${CLOUD_PROVIDER}" =~ ^(local|none)$ ]]; then
        log_success "Local storage mode - volume snapshots disabled"
    elif [[ ! "${CLOUD_PROVIDER}" =~ ^(aws|gcp|azure)$ ]]; then
        log_error "Invalid CLOUD_PROVIDER: ${CLOUD_PROVIDER}"
        log_info "Must be one of: aws, gcp, azure (or leave unset for local storage)"
        ((errors++))
    else
